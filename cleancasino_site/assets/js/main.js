...
if (contactForm) {
  ...
  // Переключатель типа контакта (email / tg), если элементы присутствуют
  ...
  const rowTg = document.getElementById('row-tg');
  if (contactTypeRadios && rowEmail && rowTg) {
    contactTypeRadios.forEach(r => r.addEventListener('change', () => {
      const val = document.querySelector('input[name="contact-type"]:checked')?.value || 'email';
      if (val === 'email') {
        rowEmail.style.display = '';
        rowTg.style.display = 'none';
      } else {
